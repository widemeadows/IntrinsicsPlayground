...
namespace IntrinsicsPlayground.Intrinsics.ArrayIntrinsics
{
    public static unsafe partial class ArrayIntrinsics
    {
        public static bool SequenceEqual_Avx(float[] array1, float[] array2)
        {
            ...
            if (array1.Length == 0) return true; // SequenceEqual_Soft(array1, array2, 0);
            ...
            fixed (float* ptr2 = &array2[0])
            {
                ...
                for (; i <= array1.Length - 8; i += 8) //16 for AVX512
                {
                    ...
                    if (!Avx.TestZ(ce, ce)) return false;
                }
        ...
        public static bool SequenceEqual_Sse(float[] array1, float[] array2)
        {
            ...
            fixed (float* ptr2 = &array2[0])
            {
                if (array1.Length < 4) return SequenceEqual_Soft(ptr1, ptr2, 0, array1.Length);

                for (; i <= array1.Length - 4; i += 4)
                {
                    var vec1 = Sse.LoadVector128(ptr1 + i);
                    var vec2 = Sse.LoadVector128(ptr2 + i);
